...
namespace lex {
    namespace {
        ...
        Token parseDoubleCharToken(Lexer& lexer, std::pair<std::string, TokenType> default_tok, std::unordered_map<std::string, TokenType> possible_pairs) {
            std::string next_char = peekChar(lexer);
            size_t line = lexer.line;
            size_t column = lexer.cursor_pos - lexer.begin_of_line;

            if (next_char.empty()) return newToken(SIS_EOF, "", line, column);

            auto it = possible_pairs.find(next_char);
            if (it != possible_pairs.end()) {
                readChar(lexer);
                return newToken(it->second, literalTokenToString(it->second), line, column); 
            }
            return newToken(default_tok.second, default_tok.first, line, column);
        }

        // TODO: implement
        Token parseSlashToken();
    ...
    Token nextToken(Lexer& lexer) {
        ...
        else if (cc == "/") return parseSlashToken();
