...
namespace lex {
  ...
  class Lexer {
    public:
      explicit Lexer(std::string input)
        :m_input(input), m_live_pos({}), m_state({}), m_buffer({})
      {
      ...
      [[nodiscard]] const Token& peekToken() const noexcept {
      ...
      [[nodiscard]] size_t getLine() const noexcept {
        return this->m_live_pos.line;
      }

      [[nodiscard]] size_t getColumn() const noexcept {
        return this->m_live_pos.column;
      }
    ...
    private:
      ...
      [[nodiscard]] size_t getAheadColumn() const noexcept {
        return this->m_state.pos - this->m_state.bol + 1;
      }
  ...
  [[nodiscard]] static bool isNum(const char& c) noexcept {
    return (c >= '0' && c <= '9');
  }
  ...
  [[nodiscard]] static bool isSpace(const char& c) noexcept {
    return (c == ' '|| c == '\t' || c == '\r' || c == '\n');
  }
} // namespace lex
